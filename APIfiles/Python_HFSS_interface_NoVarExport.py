...
def signpost_substitution(filename_in, filename_out):
    ...
    for k in range(len(Variables)):
        ...
        Unit.append(''.join(filter(lambda i: i.isalpha(), Variables[k][1])))
    
    a = np.array([Names, Value, Unit])
    variables = pd.DataFrame(a.T, columns=['Name', 'Value', 'Unit'])
    
    UnitList = ['', 'mm', 'W', 'deg'] # List of units
...
def path_and_file():
    main_path = "C:\\Users\\giannetti\\Documents\\HFSS-MATLAB_interface\\Dev7\\Python\\NoVarExport\\"
    ...
    hfss_output_filename = ["S11mag", "S11pha", "NearE"]
    hfss_output_notes = ["", "", ", False"]
...
# Optimum values
x[var2sub.iloc[:, 0] == "B"] = 59.225
x[var2sub.iloc[:, 0] == "A"] = 49.472 # 48.472
x[var2sub.iloc[:, 0] == "W"] = 4.943
x[var2sub.iloc[:, 0] == "minorA"] = 28.440
x[var2sub.iloc[:, 0] == "minorB"] = 2.014
x[var2sub.iloc[:, 0] == "minorC"] = 1.999

# Three different combinations for the power at the two ports
x[var2sub.iloc[:, 0] == "P1_power_mag"] = 2
x[var2sub.iloc[:, 0] == "P1_power_pha"] = 0
output1 = sim_hfss_matlab(x, PAF)

x[var2sub.iloc[:, 0] == "P1_power_mag"] = 4
x[var2sub.iloc[:, 0] == "P1_power_pha"] = 0
output2 = sim_hfss_matlab(x, PAF)

x[var2sub.iloc[:, 0] == "P1_power_mag"] = 8
x[var2sub.iloc[:, 0] == "P1_power_pha"] = 0
output3 = sim_hfss_matlab(x, PAF)
...
ax1.plot(outputLoad[0][1][0], outputLoad[0][1][1], label="phase")
...
for idx in range(len(outputLoad)):
    ax2.plot(outputLoad[idx][2][1], outputLoad[idx][2][2], label=str(idx + 1))

ax2.grid()
ax2.set_xlabel("Normalized position = y/(L_m + a + L + L_e)")
ax2.set_ylabel("Electric field magnitude (V/m)")
ax2.legend(["1", "2", "5"], loc="upper left", title="Power [W]")
plt.show()